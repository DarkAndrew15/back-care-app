...
interface BottomNavigationProps {
  theme?: string; // Simplificamos para evitar error de enum
}

const BottomNavigation: React.FC<BottomNavigationProps> = ({ theme = 'Purple' }) => {
  const navigate = useNavigate();
  const location = useLocation();

  const activeColor = theme === 'Mint' ? 'text-primary' : 'text-purple-main';
            ...
            >
              {isActive ? (
                <div className="relative">
                   <div className={`absolute inset-0 ${theme === 'Mint' ? 'bg-primary/30' : 'bg-purple-main/30'} blur-md rounded-full transform scale-125`}></div>
